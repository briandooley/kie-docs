...
. If you are changing {PRODUCT} from using property-based user storage to file-based user storage, apply these file updates and configuration updates. 
+
Navigate to the temporary directory where you extracted the `{PRODUCT_INIT}-$VERSION-update.zip` file and run the `kie-fs-realm-patch` script in the following format:
+
On Linux or Unix-based systems:
+
[source,subs="attributes+"]
