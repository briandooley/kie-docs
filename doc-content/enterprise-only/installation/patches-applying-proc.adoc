...
. Apply the file updates. Navigate to the temporary directory where you extracted the `{PRODUCT_INIT}-$VERSION-update.zip` file and run the `kie-fs-realm-patch` script in the following format:
+
On Linux or Unix-based systems:
+
[source,subs="attributes+"]
...
. Apply configuration updates. Navigate to the `$JBOSS_HOME` directory and run the following commands:
+
On Linux or Unix-based systems:
+
[source,subs="attributes+"]
